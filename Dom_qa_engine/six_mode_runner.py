#!/usr/bin/env python3
"""
6-Mode Experiment Runner

Tests all 6 inference graph modes from QA_ENGINE_SPEC against the same
28 PinnacleHarness questions:
  ...
  Mode R: embed -> search -> route -> classify (routed auto with BERT fallback)

Mode R uses QueryInterpreter to classify question type, ModeRouter to select
the best cognitive mode (BERT or Qwen), and falls back to the other mode
if the primary returns NOT FOUND / low confidence.
...
def run_mode(mode, conn, engine, tests):
    ...
    c.execute(f"""CREATE TABLE {table_name} (
        ...
        explained TEXT,
        qtype TEXT, routed_to TEXT, fallback_used INTEGER, rewrite_applied INTEGER
    )""")
    ...
    print(f"{'='*100}")
    if mode == "R":
        print(f"  {'ID':5s} {'Aspect':15s} {'Exp':5s} {'Act':5s} {'OK':3s} {'Conf':>8s} {'Lat':>7s} {'QType':10s} {'Route':5s} {'FB':2s} {'Question':35s} {'Answer':30s}")
        print("-" * 135)
    else:
    ...
    for test_id, aspect, question, expected_answer, expected_mode, expected_chunk_ids_json in tests:
        ...
        if not ok:
            c.execute(f"""INSERT INTO {table_name}
                (test_id, aspect, question, expected_answer, expected_mode,
                 ...
                 qtype, routed_to, fallback_used, rewrite_applied)
                VALUES (?, ?, ?, ?, ?, NULL, -999, 'ERROR', 0, 0, 0, 0, 0, 0, 0, NULL, NULL, NULL, 0, 0)""",
                (test_id, aspect, question, expected_answer, expected_mode))
            conn.commit()
        ...
        # Mode R specific fields
        qtype = result.get("qtype", "")
        routed_to = result.get("routed_to", "")
        fallback_used = 1 if result.get("fallback_used", False) else 0
        rewrite_applied = 1 if result.get("rewrite_applied", False) else 0
        ...
        if mode == "R":
            fb_str = "Y" if fallback_used else " "
            print(f"{test_id:5s} {aspect:15s} {expected_mode:5s} {actual_mode:5s} {status:3s} {confidence_display} {latency:6.0f}ms {qtype:10s} {routed_to:5s} {fb_str:2s} {question[:35]:35s} {answer[:30]:30s}")
        else:
...
def compare_all_modes(conn):
    """Print side-by-side comparison of all 6 modes."""
    c = conn.cursor()
    print("\n" + "=" * 130)
    print("6-MODE COMPARISON — SWITCHABLE COGNITION GRAPH")
    print("=" * 130)

    modes = [("A", "exp_mode_a"), ("B", "exp_mode_b"), ("C", "exp_mode_c"),
             ("D", "exp_mode_d"), ("E", "exp_mode_e"), ("R", "exp_mode_r")]
    ...
    # Mode R routing analysis
    print("\n--- MODE R ROUTING ANALYSIS ---")
    try:
        print(f"{'QType':15s} {'Count':>6s} {'BERT':>6s} {'Qwen':>6s} {'Retr':>6s} {'Fallbacks':>10s} {'ModeOK':>8s} {'AnsOK':>8s}")
        print("-" * 75)
        r_rows = c.execute("""SELECT qtype, routed_to, COUNT(*), SUM(mode_correct), SUM(answer_correct), SUM(fallback_used)
            FROM exp_mode_r GROUP BY qtype, routed_to ORDER BY qtype""").fetchall()
        qtype_stats = {}
        for qt, rt, cnt, m_ok, a_ok, fb in r_rows:
            if qt not in qtype_stats:
                qtype_stats[qt] = {"count": 0, "B": 0, "D": 0, "A": 0, "fb": 0, "m_ok": 0, "a_ok": 0}
            qtype_stats[qt]["count"] += cnt
            if rt in ("B", "D", "A"):
                qtype_stats[qt][rt] += cnt
            qtype_stats[qt]["fb"] += fb
            qtype_stats[qt]["m_ok"] += m_ok
            qtype_stats[qt]["a_ok"] += a_ok
        for qt, s in sorted(qtype_stats.items()):
            print(f"{qt:15s} {s['count']:6d} {s['B']:6d} {s['D']:6d} {s['A']:6d} {s['fb']:10d} {s['m_ok']:4d}/{s['count']:2d} {s['a_ok']:4d}/{s['count']:2d}")
    except Exception as e:
        print(f"  (routing analysis error: {e})")

    # Mode R vs D head-to-head (the key comparison)
    print("\n--- MODE R (ROUTED) vs MODE D (QWEN ONLY) — HEAD TO HEAD ---")
    try:
        rows = c.execute("""
            SELECT r.test_id, r.question, r.expected_answer, r.expected_mode,
                   r.answer as r_ans, r.confidence as r_conf, r.actual_mode as r_mode, r.mode_correct as r_ok, r.answer_correct as r_aok,
                   r.qtype, r.routed_to, r.fallback_used,
                   d.answer as d_ans, d.confidence as d_conf, d.actual_mode as d_mode, d.mode_correct as d_ok, d.answer_correct as d_aok
            FROM exp_mode_r r
            JOIN exp_mode_d d ON r.test_id = d.test_id
            WHERE r.mode_correct != d.mode_correct OR r.answer_correct != d.answer_correct
            ORDER BY r.test_id
        """).fetchall()
        if rows:
            for row in rows:
                tid, q, exp_ans, exp_mode, r_ans, r_conf, r_mode, r_ok, r_aok, qt, rt, fb, d_ans, d_conf, d_mode, d_ok, d_aok = row
                print(f"\n  {tid}: {q[:50]}")
                print(f"    Expected: '{(exp_ans or 'None')[:40]}' mode={exp_mode}")
                print(f"    R routed={rt} qtype={qt} fb={'Y' if fb else 'N'}: '{(r_ans or 'None')[:40]}' conf={r_conf:.2f} mode={r_mode} M={'OK' if r_ok else 'X'} A={'OK' if r_aok else 'X'}")
                print(f"    D (Qwen only):                          '{(d_ans or 'None')[:40]}' conf={d_conf:.2f} mode={d_mode} M={'OK' if d_ok else 'X'} A={'OK' if d_aok else 'X'}")
        else:
            print("  No differences — Mode R and Mode D produce identical results!")
    except Exception as e:
        print(f"  (comparison error: {e})")
...
def main():
    ...
    print("\n\nDONE. All 6 modes tested on same 28 questions.")
