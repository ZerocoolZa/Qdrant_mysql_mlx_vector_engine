...
| 3. REASON | not built | — |
| 4. REGRAPH | not built | — |
| 5. VALIDATE | **DONE** (SQL queries, see Fast Method) | SQL |
| 6. PLAN | not built | — |
| 7. REPAIR | **DONE** (SQL updates, see Fast Method) | SQL |
| 8. CONFIG | not built | — |
| 9. EXPORT | **DONE** (write rows to files, see Fast Method) | SQL + Python |
| 10. VERIFY | **DONE** (py_compile + VbsScanner) | Python |
...
## FAST METHOD — Step-by-Step Guide

> **This is the practical guide.** The spec above defines what the pipeline IS.
> This section defines how to RUN it in under 30 seconds instead of 45 minutes.
> **Rule:** NEVER edit files directly. Always go through the DB.

### Prerequisites

- SQLite DB at `core/Dom_Graph/dom_graph.db` (or MySQL `vb_shared`)
- Tables: `files`, `class_registry`, `method_registry`, `edges`, `config_constants`
- Python with `ast` module for ingest

### Step 1: INGEST (3 seconds)

```bash
python3 core/Dom_Graph/_ingest_eyes.py
```

Or manual SQL:
```sql
-- For each .py file:
INSERT INTO files (file_name, path, hash, size, class_count, method_count, ...)
VALUES ('eyes_26.py', '/abs/path', 'sha256hash', 18000, 30, 62, ...);

-- For each class:
INSERT INTO class_registry (file, class_name, lineno, method_count)
VALUES ('eyes_26.py', 'GhostBracket', 11, 29);

-- For each method:
INSERT INTO method_registry (class_id, method_name, lineno)
VALUES (last_insert_rowid(), 'parse_bracket_packet', 68);
```

### Step 2: FIND VIOLATIONS (2 seconds)

```sql
-- Missing Run() methods
SELECT file, class_name FROM class_registry
WHERE class_name NOT IN (
    SELECT DISTINCT cr.class_name
    FROM class_registry cr
    JOIN method_registry mr ON mr.class_id = cr.id
    WHERE mr.method_name = 'Run'
);

-- Duplicate class names
SELECT class_name, COUNT(*) as dupes, GROUP_CONCAT(file)
FROM class_registry GROUP BY class_name HAVING dupes > 1;

-- Print calls (search in source text if stored)
SELECT file_name, path FROM files WHERE file_name LIKE '%eyes_26%';

-- self._ prefix violations
SELECT class_name, method_name FROM method_registry
WHERE method_name LIKE '\_%' ESCAPE '\';

-- @staticmethod / @property / @classmethod decorators
-- (requires source text in files table or code_units)
```

### Step 3: REPAIR IN DB (3 seconds)

```sql
-- Rename a class
UPDATE class_registry SET class_name = 'GhostBracket'
WHERE class_name = 'GhostBracketUltimate' AND file = 'eyes_26.py';

UPDATE class_registry SET class_name = 'Eyes26'
WHERE class_name = 'Ghost26Eyes' AND file = 'eyes_26.py';

UPDATE class_registry SET class_name = 'Vision3D'
WHERE class_name = 'AI3DVision' AND file = 'eyes_26_v1.py';

-- Rename methods
UPDATE method_registry SET method_name = 'snapshot_subtree'
WHERE method_name = '_snapshot_subtree';

-- Delete superseded files from registry
DELETE FROM class_registry WHERE file = 'eyes_26_v2.py';
DELETE FROM class_registry WHERE file = 'eyes_26_v3.py';
DELETE FROM class_registry WHERE file = 'eyes_26_v4.py';
```

### Step 4: WRITE TO FILES (5 seconds)

```python
# For each file in DB, read the file from disk, apply the renames, write back
import sqlite3, re

conn = sqlite3.connect('core/Dom_Graph/dom_graph.db')
cur = conn.cursor()

# Get all class renames
renames = cur.execute(
    "SELECT class_name, file FROM class_registry WHERE file LIKE 'eyes_26%'"
).fetchall()

# For each file, apply string replacements
for fname in set(r[1] for r in renames):
    with open(f'core/Dom_Graph/{fname}', 'r') as f:
        content = f.read()
    # Apply renames, remove prints, fix self._, add Run() stubs
    # ... (automated by _fix_all.py script)
    with open(f'core/Dom_Graph/{fname}', 'w') as f:
        f.write(content)
```

### Step 5: VERIFY (3 seconds)

```bash
# Compile check
python3 -m py_compile core/Dom_Graph/eyes_26.py
python3 -m py_compile core/Dom_Graph/codegraph_26eyes.py
python3 -m py_compile core/Dom_Graph/eyes_26_v1.py

# VBStyle scan
PYTHONPATH=. python3 -c "
from core.utility import VbsScanner
s = VbsScanner()
for f in ['core/Dom_Graph/eyes_26.py', 'core/Dom_Graph/codegraph_26eyes.py', 'core/Dom_Graph/eyes_26_v1.py']:
    code, data, err = s.Run('scan_file', {'path': f})
    print(f.split('/')[-1] + ':', 'CLEAN' if not data else str(len(data)) + ' violations')
"

# Functional test
PYTHONPATH=. python3 -c "
from core.Dom_Graph import GhostBracket, Eyes26
e = GhostBracket('[A[B[C][D]][E[F]]]')
r = Eyes26().inspect_all(e)
print('26 eyes:', len(r), 'results')
"
```

### TOTAL TIME: ~16 seconds

Compare with manual file editing: **45 minutes**.

### When to use the Fast Method

| Task | Fast Method? | Why |
|---|---|---|
| Rename classes | YES | SQL UPDATE, then write back |
| Remove print() calls | YES | Regex on source text in DB, write back |
| Fix self._ violations | YES | SQL detect + regex fix, write back |
| Add Run() methods | YES | Template insert at class end, write back |
| Add VBStyle headers | YES | Template prepend to file, write back |
| Change method logic | NO | Complex logic changes need human/AI reasoning |
| New features | NO | Write code directly, then ingest |
| Bug fixes | NO | Find bug in code, fix directly, re-ingest |

### Key Lesson

> **The DB is for BULK OPERATIONS.** Renaming 30 classes, removing 20 prints,
> fixing 8 self._ violations — that's 58 operations. At 1 minute each manually,
> that's an hour. With SQL it's 58 UPDATE statements in one transaction:
> **3 seconds.**
>
> The file system is for WRITING CODE. The DB is for TRANSFORMING CODE.
> Ingest → Transform in DB → Write back → Verify. Never transform in the file.
