...
// VPA — Visual Performance & Analysis module
...
// Convert ImU32 to ImVec4 for TextColored
    ...
    if (ImGui::BeginMenuBar()) {
        ...
        if (ImGui::BeginMenu("Window")) {
            if (ImGui::MenuItem("Reset Layout")) {
                // Would reset docking layout
                console_log(ide, "[INFO] Layout reset (reload needed)");
            }
            ImGui::EndMenu();
        }
...
static void render_toolbar(ide_state_t *ide) {
    ...
    ImGui::TextDisabled("|");
    ImGui::SameLine();
    ImGui::TextColored(ColV4(kind_color(KIND_STEP)), "Steps: %llu", (unsigned long long)ide->bus.count_by_kind[KIND_STEP]);
    ImGui::SameLine();
    ImGui::TextColored(ColV4(kind_color(KIND_RESULT)), "Results: %llu", (unsigned long long)ide->bus.count_by_kind[KIND_RESULT]);
    ImGui::SameLine();
    ImGui::TextColored(ColV4(kind_color(KIND_ERROR)), "Errors: %llu", (unsigned long long)ide->bus.error_count);
    ImGui::SameLine();
    ImGui::TextColored(ColV4(IM_COL32(120,160,255,255)), "FPS: %.0f", ide->fps);
    ...
    if (ImGui::BeginTabBar("##left_tabs", ImGuiTabBarFlags_FittingPolicyScroll)) {
        if (ImGui::BeginTabItem("Explorer")) {
            ...
            if (ide->source.loaded) {
                ImGui::TextColored(ColV4(IM_COL32(255,200,80,255)), "Project Files");
                ImGui::Separator();
                ImGui::Text(" %s", ide->source.path);
                ImGui::Text("   %d lines", ide->source.line_count);
                ImGui::Separator();
                ImGui::TextColored(ColV4(IM_COL32(255,200,80,255)), "Classes Detected");
                for (size_t i = 0; i < ide->bus.count; i++) {
                    if (ide->bus.events[i].kind == KIND_STATE &&
                        strcmp(str_lookup(ide->bus.events[i].name), "class") == 0) {
                        ImGui::TextColored(ColV4(IM_COL32(80,255,120,255)), "  class %s", ide->bus.events[i].value.str);
                    }
                }
                ImGui::Separator();
                ImGui::TextColored(ColV4(IM_COL32(255,200,80,255)), "Functions");
                for (size_t i = 0; i < ide->bus.count; i++) {
        ...
        if (ImGui::BeginTabItem("Graph")) {
            ide->left_tab = 1;
            ImGui::TextColored(ColV4(IM_COL32(255,120,255,255)), "Knowledge Graph");
            ImGui::Separator();
            ...
            for (uint16_t s = 0; s < IDE_MAX_SOURCES; s++) {
                ...
                ImGui::TextColored(ColV4(IM_COL32(80,255,120,255)), "  %s (%d events)", str_lookup(s), ev_count);
            }
            ImGui::EndTabItem();
        }
        if (ImGui::BeginTabItem("Metrics")) {
            ide->left_tab = 2;
            ImGui::TextColored(ColV4(IM_COL32(120,160,255,255)), "Live Metrics");
            ImGui::Separator();
            ...
            if (ide->bus.timing_count > 0) {
                ImGui::TextColored(ColV4(IM_COL32(120,160,255,255)), "Timing");
        ...
        if (ImGui::BeginTabItem("Tests")) {
            ide->left_tab = 3;
            ImGui::TextColored(ColV4(IM_COL32(255,220,80,255)), "Test Results");
            ImGui::Separator();
            int total_pass = 0, total_fail = 0;
            // Imports test
            if (ide->bus.count_by_kind[KIND_IMPORT] > 0) {
                ImGui::TextColored(ColV4(IM_COL32(80,255,120,255)), "V PASS  Imports: %llu found",
                    (unsigned long long)ide->bus.count_by_kind[KIND_IMPORT]);
                total_pass++;
            } else {
                ImGui::TextColored(ColV4(IM_COL32(255,80,80,255)), "X FAIL  Imports: none");
                total_fail++;
            }
            // Per source
            for (uint16_t s = 0; s < IDE_MAX_SOURCES; s++) {
                ...
                if (errors == 0) {
                    ImGui::TextColored(ColV4(IM_COL32(80,255,120,255)), "V PASS  %s: %d facts, %d results",
                        str_lookup(s), facts, results);
                    total_pass++;
                } else {
                    ImGui::TextColored(ColV4(IM_COL32(255,80,80,255)), "X FAIL  %s: %d errors",
                        str_lookup(s), errors);
                    total_fail++;
                }
            }
        ...
        if (ImGui::BeginTabItem("Search")) {
            ide->left_tab = 4;
            ImGui::TextColored(ColV4(IM_COL32(255,220,80,255)), "Global Search");
            ImGui::Separator();
            ...
            if (ide->global_search[0]) {
                ...
                for (size_t i = 0; i < ide->bus.count && matches < 100; i++) {
                    ...
                    if (strcasestr(str_lookup(ev->source), ide->global_search) ||
                        ...
                        ImGui::TextColored(ColV4(kind_color(ev->kind)), "#%llu %s/%s/%s",
        ...
        if (ImGui::BeginTabItem("Plugins")) {
            ide->left_tab = 5;
            ImGui::TextColored(ColV4(IM_COL32(200,200,200,255)), "Plugins");
            ImGui::Separator();
...
static void render_source_editor(ide_state_t *ide) {
    ...
    for (int i = 0; i < ide->source.line_count; i++) {
        ...
        ImGui::TextColored(ColV4(IM_COL32(100,100,100,255)), "%4d", i + 1);
        ImGui::SameLine();
        ...
        // Line content
        if (has_error) {
            ImGui::TextColored(ColV4(IM_COL32(255,80,80,255)), "%s", ide->source.lines[i]);
        } else if (has_event) {
            ImGui::TextColored(ColV4(IM_COL32(80,200,255,255)), "%s", ide->source.lines[i]);
        } else {
...
static void render_event_timeline(ide_state_t *ide) {
    ...
    // Controls
    ImGui::TextColored(ColV4(IM_COL32(80,200,255,255)), "Event Timeline — %zu events", ide->bus.count);
    ImGui::SameLine();
    ImGui::TextDisabled("|");
    ImGui::SameLine();
    ...
    if (ide->bus.count > 0) {
        ...
        for (int k = 1; k <= 10; k++) {
            ...
            ImGui::TextColored(ColV4(kind_color(k)), "%s", kind_name(k));
            lx += 80;
...
static void render_graph_viewer(ide_state_t *ide) {
    ...
    ImGui::TextColored(ColV4(IM_COL32(255,120,255,255)), "Execution Graph");
    ImGui::SameLine();
    ...
    for (int i = 0; i < node_count; i++) {
        ...
        ImGui::TextColored(ColV4(border_col), "%s", sname);
        ...
        for (size_t j = 0; j < ide->bus.count && method_idx < 8; j++) {
            ...
            if (ent < IDE_MAX_SOURCES && !seen_entity[ent]) {
                ...
                ImGui::TextColored(ColV4(IM_COL32(255,220,80,255)), "%s", ename);
                method_idx++;
            }
        }
    }
...
static void render_replay_viewer(ide_state_t *ide) {
    ...
    ImGui::TextColored(ColV4(IM_COL32(80,200,255,255)), "Time-Travel Replay");
    ImGui::Separator();
    ...
    for (int i = 0; i <= ide->replay_pos && i < (int)ide->bus.count; i++) {
        ...
        if (is_current) {
            ...
            ImGui::TextColored(ColV4(IM_COL32(255,255,0,255)), ">>");
        } else {
            ImGui::TextDisabled("  ");
        }
        ImGui::SameLine();
        ImGui::TextColored(ColV4(col), "#%-4llu %-12s %-10s %-14s %-14s %s",
            (unsigned long long)ev->id, str_lookup(ev->source),
            kind_name(ev->kind), str_lookup(ev->entity),
            str_lookup(ev->name), val_str);
...
static void render_profiler(ide_state_t *ide) {
    ...
    ImGui::TextColored(ColV4(IM_COL32(120,160,255,255)), "Timing Profile");
    ImGui::Separator();
...
static void render_ai_assistant(ide_state_t *ide) {
    ...
    ImGui::TextColored(ColV4(IM_COL32(80,255,120,255)), "AI Assistant");
    ImGui::Separator();
    ...
    for (int i = 0; i < ide->ai.count; i++) {
        ...
        if (strcmp(msg->role, "user") == 0) {
            ImGui::TextColored(ColV4(IM_COL32(80,200,255,255)), "You: %s", msg->text);
        } else {
            ImGui::TextColored(ColV4(IM_COL32(80,255,120,255)), "AI: %s", msg->text);
        }
        ImGui::Spacing();
    }
    // Auto-scroll
    ...
    if (ImGui::BeginTabBar("##bottom_tabs")) {
        ...
        if (ImGui::BeginTabItem("Events")) {
            ...
            for (size_t i = 0; i < ide->bus.count; i++) {
                ...
                char id_buf[32]; snprintf(id_buf, sizeof(id_buf), "%llu", (unsigned long long)ev->id);
                if (ImGui::Selectable(id_buf, selected,
                    ImGuiSelectableFlags_SpanAllColumns)) {
                    ide->selected_event = (int)i;
                }
                ImGui::NextColumn();
                ImGui::TextColored(ColV4(col), "%s", str_lookup(ev->source)); ImGui::NextColumn();
                ImGui::TextColored(ColV4(col), "%s", kind_name(ev->kind)); ImGui::NextColumn();
        ...
        if (ImGui::BeginTabItem("Variables")) {
            ...
            for (size_t i = 0; i < ide->bus.count; i++) {
                ...
                ide_event_t *ev = &ide->bus.events[i];
                char val_str[128]; ValFormat(&ev->value, val_str, sizeof(val_str));
                ImGui::TextColored(ColV4(IM_COL32(255,220,80,255)), "%s = %s", str_lookup(ev->name), val_str);
            }
            ImGui::EndChild();
            ImGui::EndTabItem();
        }
        ...
        if (ImGui::BeginTabItem("State")) {
            ...
            for (size_t i = 0; i < ide->bus.count; i++) {
                ...
                ide_event_t *ev = &ide->bus.events[i];
                char val_str[128]; ValFormat(&ev->value, val_str, sizeof(val_str));
                ImGui::TextColored(ColV4(IM_COL32(255,120,255,255)), "%s.%s = %s",
        ...
        if (ImGui::BeginTabItem("Timeline")) {
            ...
            for (size_t i = 0; i < ide->bus.count; i++) {
                ...
                ImGui::TextColored(ColV4(kind_color(ev->kind)), "[%.6f] #%llu %s/%s/%s",
        ...
        if (ImGui::BeginTabItem("Errors")) {
            ...
            for (size_t i = 0; i < ide->bus.count; i++) {
                ...
                ide_event_t *ev = &ide->bus.events[i];
                char val_str[128]; ValFormat(&ev->value, val_str, sizeof(val_str));
                ImGui::TextColored(ColV4(IM_COL32(255,80,80,255)), "X #%llu [%s/%s] %s: %s",
                    (unsigned long long)ev->id, str_lookup(ev->source),
                    str_lookup(ev->phase), str_lookup(ev->name), val_str);
                err_count++;
            }
            if (err_count == 0)
                ImGui::TextColored(ColV4(IM_COL32(80,255,120,255)), "No errors.");
            ImGui::EndChild();
            ImGui::EndTabItem();
        }
...
static void render_status_bar(ide_state_t *ide) {
    ...
    if (ImGui::GetCursorPosX() < right) {
        ...
        if (ide->bus.error_count > 0) {
            ImGui::TextColored(ColV4(IM_COL32(255,80,80,255)), " HAS ERRORS ");
        } else {
            ImGui::TextColored(ColV4(IM_COL32(80,255,120,255)), " ALL PASS ");
        }
    }
...
static void render_about(ide_state_t *ide) {
    ...
    if (ImGui::BeginPopupModal("About Bloodhound", &ide->show_about,
        ImGuiWindowFlags_AlwaysAutoResize)) {
        ImGui::TextColored(ColV4(IM_COL32(80,200,255,255)), "Bloodhound IDE v%s", IDE_VERSION);
        ImGui::Separator();
...
int main(int argc, char **argv) {
    ...
    // Init VPA module (ImPlot, ImNodes, TextEditor)
    VPA_Init();
    ...
    while (!glfwWindowShouldClose(window) && ide->running) {
        ...
        // VPA panels — Visual Performance & Analysis
        VPA_RenderLiveDashboard((void*)ide);
        VPA_RenderPerformanceGraphs((void*)ide);
        VPA_RenderNodeGraph((void*)ide);
        VPA_RenderCodeEditor((void*)ide);
        VPA_RenderKindDistribution((void*)ide);
        VPA_RenderEventRateGraph((void*)ide);
        VPA_RenderMemoryGraph((void*)ide);
        VPA_RenderAIReasoningGraph((void*)ide);
