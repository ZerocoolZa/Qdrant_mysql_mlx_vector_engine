...
#[@REVIEW]{[@date<2026-06-29>][@reviewer<devin>][@status<pass>][@notes<Multi-hop relationship follower. chat to file to class to BCL to rule to chat traversal. VBStyle: Run dispatch, Tuple3, self.state. No shebang line. No violations visible.>][@todos<none>]}
...
# ════════════════════════════════════════════
# MODULE CONSTANTS — node ranking weights
# ════════════════════════════════════════════
...
class MagneticGraph:
    ...
    def conn(self, database="vb_shared"):
    ...
    def Run(self, command, params=None):
        dispatch = {
            ...
            "rank": self.cmd_rank,
            "class_graph": self.cmd_class_graph,
            "callers": self.cmd_callers,
            "callees": self.cmd_callees,
            "dependencies": self.cmd_dependencies,
            "chat_hits": self.cmd_chat_hits,
    ...
    def cmd_traverse(self, params):
        query = self.p(params, "query")
        max_hops = self.p(params, "max_hops", self.state["config"]["max_hops"])
        radius = self.p(params, "radius", self.state["config"]["radius"])
        limit = self.p(params, "limit", self.state["config"]["limit"])
        if not query:
    ...
    def find_chat_hits(self, keyword, radius, limit):
        ...
        try:
            ...
            for row in c.fetchall():
                ...
                files = self.extract_file_paths(content)
    ...
    def rank_nodes(self, neighborhood):
        ...
        for node_type, items in nodes.items():
            if not isinstance(items, list):
                continue
            type_weight = NODE_TYPE_WEIGHTS.get(node_type, 0.50)
            hop_num = 1
            for h in hops:
            ...
            for item in items:
                if not isinstance(item, dict):
                    continue
                score = type_weight
                score -= hop_num * HOP_DISTANCE_PENALTY
    ...
    def cmd_rank(self, params):
        ...
        query = self.p(params, "query")
        if query:
            ok, data, err = self.cmd_traverse(params)
            if not ok:
                return (0, None, err)
            return (1, {
                "query": query,
    ...
    def cmd_class_graph(self, params):
        class_name = self.p(params, "class_name") or self.p(params, "query")
        limit = self.p(params, "limit", self.state["config"]["limit"])
        if not class_name:
    ...
    def cmd_callers(self, params):
        method = self.p(params, "method") or self.p(params, "query")
        limit = self.p(params, "limit", self.state["config"]["limit"])
        if not method:
    ...
    def cmd_dependencies(self, params):
        method = self.p(params, "method") or self.p(params, "query")
        limit = self.p(params, "limit", self.state["config"]["limit"])
        if not method:
            return (0, None, ("ERR_PARAMS", "method required", 0))
    ...
    def cmd_chat_hits(self, params):
        keyword = self.p(params, "query") or self.p(params, "keyword")
        radius = self.p(params, "radius", self.state["config"]["radius"])
        limit = self.p(params, "limit", self.state["config"]["limit"])
        if not keyword:
            return (0, None, ("ERR_PARAMS", "query required", 0))
