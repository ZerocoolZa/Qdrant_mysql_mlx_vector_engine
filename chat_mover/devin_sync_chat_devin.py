#!/usr/bin/env python3
"""Devin Sync → CHAT_DEVIN. Streams SQLite to MySQL with constraint-driven dedup.
No Python dedup sets. MySQL unique indexes enforce uniqueness. No DELETE/DROP/TRUNCATE.
Usage: python devin_sync_chat_devin.py [--watch|--status|--verify|--dry-run]"""
...
MYSQL = dict(host="localhost", user="root", database="CHAT_DEVIN", autocommit=True, connection_timeout=10)
...
def mcur(conn):
    """Prepared cursor — fixes mysql.connector escaping bug on certain content."""
    return conn.cursor(prepared=True)


def check_constraints(m):
    """RULE 4: refuse to run if unique indexes don't exist."""
    required = {
    ...
    for table, indexes in required.items():
        m.execute(f"SHOW INDEX FROM {table}")
        have = {r[2] for r in m.fetchall()}
        for idx in indexes:
            if idx not in have:
                raise RuntimeError(f"RULE 4 VIOLATION: {table} missing index {idx}. Refusing to run.")
...
def sync_tools(s, m):
    try: data = stream(s, "tool_call_state")
    except sqlite3.DatabaseError: print("  tool_calls:       table missing"); return
...
def run_sync():
    print("=" * 60)
    print(f"Sync → CHAT_DEVIN — {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}{' [DRY RUN]' if DRY else ''}")
    print("=" * 60)
    sdb = mdb = None
    try:
        mdb = mconn(); mcur = mdb.cursor(prepared=True)
        check_constraints(mcur)
        print("  [ok] constraint check passed")
        sdb = recover()
...
def show_status():
    db = mconn(); cur = db.cursor(prepared=True)
    ...
    print("=" * 50); print("CHAT_DEVIN STATUS"); print("=" * 50)
    for t in tables:
...
def verify(s, m):
    print("=" * 50); print("VERIFY — SQLite vs CHAT_DEVIN"); print("=" * 50)
    for st, mt in [("sessions", "devin_sessions"), ("message_nodes", "devin_messages"), ("rendered_commits", "devin_rendered_commits")]:
        try:
            ...
            print(f"  {st:20s} sqlite={sc:>8,}  chat_devin={mc:>8,}  {'OK' if sc <= mc else 'MISMATCH'}")
...
def main():
    if "--status" in sys.argv: show_status()
        ...
        if sdb and mdb: verify(sdb.cursor(), mdb.cursor(prepared=True))
        try: sdb.close()
        except: pass
        try: mdb.close()
        except: pass
